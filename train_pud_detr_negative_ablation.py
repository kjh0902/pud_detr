#!/usr/bin/env python3
"""Train PN negative-weight ablations or the historical PUD-DETR implementation.
...
# PN ablation: --method pn --negative-weight 0.5 scales all target=0
# (query, class) focal losses, including non-target classes of matched queries.
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    experiment.add_argument(
        ...
        default=1.0,
        help=(
            "PN only: scale focal loss at every target=0 (query, class) pair; "
            "1.0 preserves the original PN loss."
        ),
    )
    experiment.add_argument(
        "--weight-p",
...
def validate_args(parser: argparse.ArgumentParser, args: argparse.Namespace) -> None:
    ...
    if not 0 <= args.negative_weight <= 1:
        parser.error("--negative-weight must be in [0, 1]")
    if args.method != "pn" and args.negative_weight != 1.0:
        parser.error("--negative-weight other than 1.0 requires --method pn")
    if args.weight_p <= 0:
...
def pn_sigmoid_focal_loss(
    ...
    if negative_weight != 1.0:
        # Include non-target classes of matched queries as well as unmatched queries.
        loss = torch.where(targets == 0, loss * negative_weight, loss)
